...
#homeownership and application type are factors 

class(loans_full_schema$interest_rate)
#interest rate is numeric 
```
...
loans <- loans_full_schema %>% 
  mutate(
    homeownership = tolower(homeownership), #lower case 
    homeownership = fct_relevel(homeownership, "rent", "mortgage", "own"),
    application_type = fct_relevel(application_type, "joint", "individual")
  )
levels(loans$homeownership) #check that the factor levels were reordered 
```
...
#bar plot showing the total counts of loans in each homeownership category 
p_count <- ggplot(loans, aes(x = homeownership)) + 
  geom_bar(fill = IMSCOL["green", "full"]) + #variable is always on the x axis 
  labs(x = "Homeownership", 
       y = "Count")
#IMSCOL = matrix which contains color hex codes for colors frequently used in IMS book 

#bar plot showing the proportion of loans in each homeownership category 
#tells us more about data distribution 
p_proportion <- loans %>% 
  count(homeownership) %>% 
  mutate(proportion = n / sum(n)) %>% #create the proportion 
  ggplot(aes(x = homeownership, y = proportion)) + 
  geom_col(fill = IMSCOL["red", "f3"]) + #allows us to choose which variable goes on x and y axes 
  labs(x = "Homeownership", 
       y = "Proprtion") 
...
loans %>%
  ...
  # counts observations for each application_type X homeownership combination, creating a column called "n"
  pivot_wider(names_from = homeownership, values_from = n) %>% 
  # names_from: What is the output column that you want called? values_from: What column should the cell values come from?
  select(application_type, rent, mortgage, own) %>% 
  # select the columns you want (here, we're taking all of them)
  adorn_totals(where = c("row", "col")) %>% 
  # add "Totals" for both rows and coluns
  kbl(linesep = "", booktabs = TRUE, caption = "A contingency table for application type and homeownership.") %>% 
  # kable makes tables look fancy 
  kable_styling(bootstrap_options = c("striped", "condensed"), 
                latex_options = c("striped", "hold_position"), full_width = FALSE) %>%
  add_header_above(c(" " = 1, "homeownership" = 3, " " = 1)) %>%
  column_spec(1:5, width = "6em") 
# select columns, specify preferred formatting
```
...
p_stacked <- ggplot(loans, aes(x = homeownership, fill = application_type)) + #fill based on variables 
  geom_bar() +
  scale_fill_manual(values = c(IMSCOL["blue", "full"], IMSCOL["yellow", "full"])) + #choose fill color values 
...
p_dodged <- ggplot(loans, aes(x = homeownership, fill = application_type)) +
  ...
  axis.title = element_text(size=20,face="bold"))
p_dodged
#shows us there are few joint applicants
#but this is still count data, which is not very helpful 
```
...
p_standardized <- ggplot(loans, aes(x = homeownership, fill = application_type)) +
  geom_bar(position = "fill") + #to create the proportions 
  scale_fill_manual(values = c(IMSCOL["blue", "full"], IMSCOL["yellow", "full"])) +
  ...
  axis.title = element_text(size=20,face="bold"))
p_standardized
#proportion data - more helpful than count data 
```
