...
```{r}
#summary(lm(head_l ~ tail_l, data = possum))
#summary(lm(total_l ~ tail_l, data = possum))

head_mod <- lm(head_l ~ tail_l, data = possum)
summary(head_mod)$r.squared #R2 value 
head_mod$coefficients #intercept + slope 

tot_mod <- lm(total_l ~ tail_l, data = possum)
```

2.  Which regression model fits the data better? Is this what you expected based on the Scatter plots? Why or why not?

The total length model fits the data better with a $R^2$ value of 0.32. This is expected based on the scatter plot because there was a stronger correlation between the total length and tail length data.

Notes: - "regress y on x" (Ex. regress total length on tail length) - $R^2$ = 1 - (var(e)/var(y)) = % of variance explained by the model - correlation = normalization of covariance = -1 to +1
...
```{r}
head(mpg)
class(mpg$year) #year is an integer - we want it as a factor 
mpg <- mpg %>% 
  mutate(year = as.factor(year)) #convert year to factor 
class(mpg$year) #double check if it is a factor 
```
...
The fuel efficiency distributions look very similar between the two groups. There is a lot of overlap in the middle 50% of the data.
...
nine <- subset(mpg, year == 1999)
eight <- subset(mpg, year == 2008)
mean(nine$hwy)
mean(eight$hwy)
```
...
```{r}
car <- lm(hwy ~ year, data = mpg)
summary(car)
#R creates an dummy variable for categorical variables like year 
#1999 becomes the reference group 
```

2.  What does the intercept tell you, in words?

For the reference group 1999 (when the dummy variable 2008 is turned off), the average highway mpg = 23.4274.

3.  What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words?

For 2008 (when the dummy variable 2008 is turned on), the average highway mpg = intercept + (slope \* 1) = 23.4274 + 0.0256 = 23.5mpg.

4.  Does your model suggest anything about whether fuel efficiency has evolved over time? Why or why not?

Our model does not suggest any relationship between fuel efficiency and vintage year (p-value \>\>\> 0.05). Similarly, the model explains very little of the variation in y ($R^2$ = 4.66e-6)
...
ggplot(data = mpg, aes(x = displ, y = hwy))+
  ...
  theme_classic()+
  labs(x = "Engine Size", 
       y = "Fuel Efficiency (highway mpg)")

#negative correlation between engine size and hwy mgp 
```
...
```{r}
eng_mod <- lm(hwy ~ displ + year, data = mpg)
summary(eng_mod)
#year = categorical variable
#displ = numeric variable 
```


2.  Interpret your three coefficients, paying careful attention to units.

For the reference group 1999 (when the dummy variable 2008 is turned off) and when displ = 0, the average highway mpg = 35.276.
- $\hat\beta_0$ = 35.276 
- $\hat\beta_1$ = 0
- $\hat\beta_2$ = 0 (dummy turned off )

For every unit increase of displ, highway mpg decreases by on average 3.611. 
- $\hat\beta_0$ = 35.276 
- $\hat\beta_1$ = -3.611 
- hold fixed the years variable 

For 2008 (when the dummy variable 2008 is turned on) and when displ = 0, the average highway mpg = 35.276 + 1.402 = 36.7mpg. 
- $\hat\beta_0$ = 35.276 
- hold fixed the displ variable 
- $\hat\beta_2$ = 1 (turn on the dummy)
-- once we hold fixed engine sizes, we can see significant impacts of years on fuel efficiency, which were obscured before 
...
2.  Second, add two regression lines to the plot, one that shows the predicted relationship between miles per gallon and engine size for the 1999 vintage, and a second that shows the same predicted relationship but for the 2008 vintage.[^3]
...
ggplot(data = mpg, aes(y = hwy, x = displ))+ 
  ...
  geom_smooth(method = "lm", 
              ...
              aes(color = year))+
  theme_classic()
#one slope coefficient + 2 categories 
#illustrate the difference in means 
```


[^3]: Hint: The variable `.fitted` gives you the predicted (or "fit") values for all observations in the data.
...
augment(mod)
#.fitted = predictions of y hat of x <- we can now plot the predicted line of best fit with these points 
#.resid = residuals
```
...
ggplot(data = mpg, aes(x = displ, y = hwy))+ 
  ...
  theme_classic()+ 
  labs(x = "Engline Displacement (Liters)", 
       y = "Highway Miles per Gallon")
```
