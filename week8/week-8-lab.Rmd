...
```{r}
data <- readRDS("week-eight-data.RDS")
head(data)
#x + y = some sort of coordinate system like lat/long (2D)
#z = well depth m (3D)
```
...
- the data are evenly spatially distributed 
...
```{r}
#in general, sf is a good place to start using R's spatial functionality 

#convert df -> sf object 
#choose columns to set as X and Y coordinates in the geometry column 
#cbind to keep the original X and Y columns 
...
```{r}
#gstat is key for kriging in R 
#we will focus on ordinary kriging - fixed mean but we don't know what it is 
#1 = intercept term 

var <- gstat::variogram(
  Z~1,
  as(data_sf, "Spatial") #convert sf -> sp 
)

plot(var)

#as distance increases, the variance increases 
#we don't see a plateau 
#it builts it out as far as H can go given the data 
#given our data, the spatial correlation is large compared to our sample - we don't have data that is spatially distributed far enough out to see the leveling off 
#we don't see a nugget effect -> we're hitting zero variance at zero distance (there's no discrete jump from the nugget)

#each dot drew a donut to compute the variance that sit H units apart 
#this is an empirical variogram 
```
...
```{r}
#next, we fit a line to the dots (variogram model) 
#we can specify the model we want or R will fit the best model for us 

mod_full <- automap::autofitVariogram(Z~1, as(data_sf, "Spatial"))
mod <- mod_full$var_model
```
...
```{r}
plot(mod_full)
#the full model fits our variogram data very well 
...
```{r}
#we need to define a grid onto which we can put our raster 
#what resolution of raster do we want to interpolate onto? 
#coarse grid will run faster 

grid <- data_sf %>% 
  st_bbox() %>% #gets bounding box of our observations
  st_as_sfc() %>% #makes bbox an sfc object
  st_make_grid( #make the grid 
    cellsize = c(100,100),
    what = "centers" #we want geometries associated w/center of grid points 
  ) %>% 
  st_as_sf() %>% #return grid to sf object
  cbind(., st_coordinates(.)) #retain X and Y coordinates 
plot(grid)
#colors represent how X and Y are increasing 
```
...
```{r}
grd_100_sp <- as(grid, "Spatial") # converting to {sp} format
...
OK <- krige(
  ...
  as(data_sf, "Spatial"), # input data in {sp} format
  grd_100_sp,                # locations to interpolate at
  model = mod           # the variogram model fitted above
  )
...
head(OK$var1.pred) #predictions
head(OK$var1.var) #uncertainty 
```
