---
title: "EDS 222: Week 3 Ordinary Least Squares Lab"
author: "Mia Forsline"
date: "`r Sys.Date()`"
---
...
Tail length and total body length are positively correlated. Tail length and head length are also positively correlated, but not as strongly as tail length and total body length because there is more noise in the data. 

```{r}
ggplot()+
  geom_point(data = possum, aes(x = tail_l, y = total_l))+ 
  theme_classic()+
  labs(x = "Possum Tail Length (cm)",
       y = "Possum Total Length (cm)")
...
1. Use the plots from section 1 to write your best estimate of the correlation coefficient between each of the two examples in section 1. 
  (i) the length of a possum's total length and the length of possum tail: 0.80 
  (ii) the length of a possum's head and the length of possum tail: 0.30
...
cor(possum$tail_l, possum$total_l, method = c("pearson"))
...
#correlations normalize the relationship between two variables so it ranges from 0-1 
...
```{r}
output <- lm(formula = total_l ~ tail_l, data = possum)
summary(output)
```
...
Possum body length and possum tail length have a significant association (p <<< 0.05). For every cm of possum total length, possum tail length increases by 1.24 cm. 

3. Is your estimate of $\beta_0$ directly interpretable? What might you be concerned about when interpreting $\beta_0$? 

$\beta_0$ = 41.04. So when possum possum tail length = 0, possum body length = 41.04. However, this value is not biologically meaningful because possum tail length never zero. Though the intercept is helpful to create the line of best fit, it is best to only interpret the line within the range of the data.  
...
Yes, our model suggests that tail length and body length also have a small positive correlation because the there was a slope of 1.24. If there was no relationship between tail length and body length, we would expect a slope of 1. Since the slope > 1, body length must also be increasing as tail length increases. 
...
```{r}
output <- lm(formula = head_l ~ tail_l, data = possum)
summary(output)
...
```{r}
model <- lm(total_l ~ tail_l, data = possum)
predictions <- possum %>% 
  add_predictions(model) %>% #insert regression model into add_predictions()
  mutate(residuals = total_l - pred) #actual data minus predicted line 
```
...
```{r}
ggplot(data = predictions, aes(x = residuals))+
  geom_histogram()
```
...
```{r}
ggplot(data = predictions, aes(x = tail_l, y = residuals))+
  geom_point()+
  theme_classic()
```
...
```{r}
output1 <- lm(formula = total_l ~ tail_l, data = possum)
summary(output1)

output2 <- lm(formula = head_l ~ tail_l, data = possum)
summary(output2)
```
