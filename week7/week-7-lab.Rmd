...
$$H_{0}:$$ There is no difference in average birth weight between babies born to mothers that smoke versus mothers that do not smoke. 

$$H_{A}:$$ There is a difference in average birth weight between babies born to mothers that smoke versus mothers that do not smoke. 

- we are interested in 2 means 
...
```{r}
mu_nonsmoker <- ncbirths_complete_habit %>%  
  filter(habit == "nonsmoker") %>% 
  summarize(mean(weight))
mu_smoker <- ncbirths_complete_habit %>%  
  filter(habit == "smoker") %>% 
  summarize(mean(weight))
...
print(point_est)
#units = lbs 
#non-smoker mothers have slightly heavier babies on avg
#is there a true relationship between birthweight and smoking ? or did we by chance happen to get a sample where non-smoker mothers had heavier babies on average ? 
```
...
#n = sample size
#s = SD of the sample 
n1 <- ncbirths_complete_habit %>% 
  filter(habit == "nonsmoker") %>%  
  count()
n2 <- ncbirths_complete_habit %>% 
  filter(habit == "smoker") %>%  
  count()
s1 <- ncbirths_complete_habit %>% 
  filter(habit == "nonsmoker") %>%  
  summarize(sd(weight, na.rm = TRUE))
s2 <-  ncbirths_complete_habit %>% 
  filter(habit == "smoker") %>%  
  summarize(sd(weight, na.rm = TRUE))
...
se
#SE tells us about the spread of the sampling distribution of the mean birthweights 
#point_estimate = mu_nonsmoker - mu_smoker <- from this we can make a sampling distribution 
#SE helps us estimate the true SD of the true pop (which we can never do because we cannot sample thousands of time)
#instead, we have one sample to calculate the SE 
```
...
zscore
#null value = 0 
#zscore = point_estimate (mean difference between smoker vs non-smoker = 0.3lb) is 2.36 standard deviations away from the null (zero) 
#score > 2, so we are pretty far into the tail of the sampling distribution 
```
...
```{r}
#we want to compute the area under the curve past 2.36 SD and -2.36 SD in the sampling distribution 
#pnorm() calculates the area to the left by default -> but we can use lower.tail = FALSE to flip it 
pvalue <- 2 * pnorm(q = point_est, mean = 0, sd = se, lower.tail = FALSE)
pvalue
#or we could pass through q = zscore + sd = 1 
## zscore = # of SD away from the mean on a standard normal distribution (mean = 0, sd = 1)

#q = quantile = cut off (in this case, our z-score/test statistic/point estimate)
#mean = 0 (null)
#sd = SE (SE = SD of the difference between smokers vs nonsmokers - sampling distribution, not the data distribution)
## sampling distribution = we can never truly sample it because we cannot sample the population thousands of times
## data distribution = we have the data 
#SE = our approximation of SD if we could sample the pop thousands of times 
```
...
```{r}
t.test(weight ~ habit, data = ncbirth_complete_habit)
```
...
```{r}
t.test(ncbirths_complete_habit$weight[ncbirth_complete_habit$habit == "nonsmoker"], ncbirths_complete_habit$weight[ncbirth_complete_habit$habit == "smoker"])
#another way to format the t-test 
#passing through vectors of data 
#separated by comma rather than ~ z
```
